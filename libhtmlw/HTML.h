...
typedef struct image_rec {	/* location-invariant info about an image */
	int internal;
	int delayed;
	int fetched;
	int width, height;
	...
	Pixmap image;
	const char *name;
	struct image_rec *next;
} ImageInfo;


typedef struct img_rec {	/* HTML modifiers regarding an image */
	ImageInfo *pic_data;	/* -> generic image info */
	...
	/* int width, height; if <img width=> override is ever supported */
} IMGInfo;
...
/*
 * Public functions (at the end so we can use types defined above)
 */
extern void	 BuildInternalImagePixmaps _ArgProto((HTMLWidget hw));
extern int	 hb  _ArgProto((unsigned long ul));
extern int	 HTMLAnchorToPosition _ArgProto((Widget w, char *name,
						 int *x, int *y));
extern int	 HTMLAnchorToId _ArgProto((Widget w, char *name));
extern void	 HTMLClearSelection _ArgProto((Widget w));
extern char **   HTMLGetHRefs _ArgProto((Widget w, int *num_hrefs));
extern void	 HTMLGetImage _ArgProto((HTMLWidget hw, struct ele_rec *eptr,
					 Boolean now));
extern char **   HTMLGetImageSrcs _ArgProto((Widget w, int *num_srcs));
extern LinkInfo *HTMLGetLinks _ArgProto((Widget w, int *num_links));
extern char *    HTMLGetText _ArgProto((Widget w, int pretty));
extern char *    HTMLGetTextAndSelection _ArgProto((Widget w, char **startp,
						    char **endp,
						    char **insertp));
extern void	 HTMLGotoId _ArgProto((Widget w, int element_id));
extern ImageInfo *HTMLResolveDelayedImage _ArgProto((HTMLWidget hw,
						     const char *const edata));
extern void	 HTMLRetestAnchors _ArgProto((Widget w,
					      visitTestProc testFunc));
extern void	 HTMLSetSelection _ArgProto((Widget w, ElementRef *start,
					     ElementRef *end));
extern void	 HTMLSetText _ArgProto((Widget w, char *text,
					char *header_text, char *footer_text));
extern int	 HTMLSearchText _ArgProto((Widget w, char *pattern,
					   ElementRef *m_start,
					   ElementRef *m_end,
