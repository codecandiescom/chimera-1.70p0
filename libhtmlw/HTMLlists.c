...
struct mark_up *List;
{
  ...
  while (current != NULL)
  {
    ...
    if (mptr->start != NULL)
    {
      XtFree((char *) mptr->start);
    }
    if (mptr->text != NULL)
    {
      XtFree((char *) mptr->text);
    }
    if (mptr->end != NULL)
    {
      XtFree((char *) mptr->end);
    }
    XtFree((char *) mptr);
  }
}
...
int type;
{
  switch (type)
  {
  case M_NONE:		printf ("M_NONE");	break;
  case M_TITLE:		printf ("M_TITLE");	break;
  case M_FIXED:		printf ("M_FIXED");	break;
  case M_BOLD:		printf ("M_BOLD");	break;
  case M_ITALIC:	printf ("M_ITALIC");	break;
  case M_EMPHASIZED:	printf ("M_EMPHASIZED"); break;
  case M_STRONG:	printf ("M_STRONG");	break;
  case M_CODE:		printf ("M_CODE");	break;
  case M_SAMPLE:	printf ("M_SAMPLE");	break;
  case M_KEYBOARD:	printf ("M_KEYBOARD");	break;
  case M_VARIABLE:	printf ("M_VARIABLE");	break;
  case M_CITATION:	printf ("M_CITATION");	break;
  case M_STRIKEOUT:	printf ("M_STRIKEOUT"); break;
  case M_HEADER_1:	printf ("M_HEADER_1");	break;
  case M_HEADER_2:	printf ("M_HEADER_2");  break;
  case M_HEADER_3:	printf ("M_HEADER_3");  break;
  case M_HEADER_4:	printf ("M_HEADER_4");  break;
  case M_HEADER_5:	printf ("M_HEADER_5");	break;
  case M_HEADER_6:	printf ("M_HEADER_6");  break;
  case M_ANCHOR:	printf ("M_ANCHOR");	break;
  case M_PARAGRAPH:	printf ("M_PARAGRAPH");	break;
  case M_ADDRESS:	printf ("M_ADDRESS");	break;
  case M_PLAIN_TEXT:	printf ("M_PLAIN_TEXT"); break;
  case M_LISTING_TEXT:	printf ("M_LISTING_TEXT"); break;
  case M_UNUM_LIST:	printf ("M_UNUM_LIST"); break;
  case M_NUM_LIST:	printf ("M_NUM_LIST");	break;
  case M_MENU:		printf ("M_MENU");	break;
  case M_DIRECTORY:	printf ("M_DIRECTORY"); break;
  case M_LIST_ITEM:	printf ("M_LIST_ITEM"); break;
  case M_DESC_LIST:	printf ("M_DESC_LIST"); break;
  case M_DESC_TITLE:	printf ("M_DESC_TITLE"); break;
  case M_DESC_TEXT:	printf ("M_DESC_TEXT"); break;
  case M_IMAGE:		printf ("M_IMAGE");	break;
  case M_SELECT:	printf ("M_SELECT");	break;
  case M_OPTION:	printf ("M_OPTION");	break;
  case M_INPUT:		printf ("M_INPUT");	break;
  case M_TEXTAREA:	printf ("M_TEXTAREA");	break;
  case M_FORM:		printf ("M_FORM");	break;
  case M_INDEX:		printf ("M_INDEX");	break;
  case M_HRULE:		printf ("M_HRULE");	break;
  case M_BASE:		printf ("M_BASE");	break;
  case M_LINEBREAK:	printf ("M_LINEBREAK");	break;
  case M_BLOCKQUOTE:	printf ("M_BLOCKQUOTE"); break;
  case M_TABLE:		printf ("M_TABLE");	break;
  case M_TABLE_ROW:	printf ("M_TABLE_ROW"); break;
  case M_TABLE_ITEM:	printf ("M_TABLE_ITEM"); break;
  case M_CAPTION:	printf ("M_CAPTION");	break;
 ...
 * (FreeLineList and MakeLineList are completely unrelated functionally.)
 */
void
FreeLineList (list)
struct ele_rec *list;
{
  struct ele_rec *current = list;

  while (current != NULL)
  {
    struct ele_rec *eptr = current;

    current = current->next;
    eptr->next = NULL;
    if (eptr->edata != NULL)
    {
      XtFree((char *) eptr->edata);
    }
    if (eptr->anchorHRef != NULL)
    {
      XtFree((char *) eptr->anchorHRef);
    }
    if (eptr->anchorName != NULL)
    {
      XtFree((char *) eptr->anchorName);
    }
    XtFree((char *) eptr);
  }
}
...
int max_line;
{
   ...
   * malloc index array
   */
  ll = (struct ele_rec **)XtMalloc(sizeof (struct ele_rec *) * max_line);
...
/*
 * The AddHRef/FindHRef mechanisms aren't used by Chimera
 */
#if 0
/*
...
char *href;
{
  ...
  if (hptr == NULL)
  {
    hptr = (struct ref_rec *)XtMalloc(sizeof(struct ref_rec));
    hptr->anchorHRef = XtNewString(href);
    hptr->next = list;
    list = hptr;
  }
