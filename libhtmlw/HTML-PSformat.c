...
static int PShex 
ARG2 (unsigned char, val, int, flush)
{
...
ARG2 (char *, title, int, font)
{
  ...
  static char *txt[] =
  {
    ...
    "%%Creator: NCSA HTML widget, Postscript by Ameet Raval & Frans van Hoesel",
    "%%Pages: (atend)",
    "%%EndComments",
    "save",
    "/D {def} def /E {exch} D",
...
ARG2 (String, t, int, underline)
{
  ...
  t2 = (String) XtMalloc ((tp - t) + nspecial + 3 * nisochar + 1);
...
static void PSbullet 
ARG2 (int, level, int, size)
{
...
static int PSrle_encode 
ARG3 (unsigned char *, scanline,
...
static int PSwrite_bw 
ARG4 (unsigned char *, pic, int, w, int, h, int, flipbw)
{
...
String 
ParseTextToPSString (hw, startp, endp, start_pos, end_pos,
...
int fontfamily;
{
  ...
  while ((eptr != NULL) && (eptr != end))
  {
	...
	PSimage (eptr->img_data->pic_data, (eptr->anchorHRef != NULL));
	break;
      }
